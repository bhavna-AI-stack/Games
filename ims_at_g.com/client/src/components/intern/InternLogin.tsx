...
import { Lock, Mail, Loader2, ArrowLeft, KeyRound, CheckCircle2, Send, Eye, EyeOff } from "lucide-react";
...
export default function InternLogin() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      setLocation('/intern/dashboard');
    } catch (error) {
      toast({
        title: "Login Failed",
        description: error instanceof Error ? error.message : "Invalid credentials or account not approved",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <DialogContent className="max-w-md">
          {!resetSuccess ? (
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 h-12 bg-background/50 border-border/50 focus:border-purple-500 transition-colors"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                ...
                <Button
                  ...
                  className="w-full h-12 bg-gradient-to-r from-purple-500 to-blue-600 hover:from-purple-600 hover:to-blue-700 text-white font-semibold shadow-lg shadow-purple-500/30 transition-all"
                >
                  {isSendingReset ? (
                    <>
            ...
            <div className="text-center py-8 space-y-6">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center mx-auto shadow-lg shadow-green-500/50 animate-in zoom-in-95">
